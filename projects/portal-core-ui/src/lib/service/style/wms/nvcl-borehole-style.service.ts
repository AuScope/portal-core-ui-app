...
export class NVCLBoreholeStyleService {
  ...
  public static getSld(layerName: string, styleName: string, param: NVCLBoreholeStyleParams): string {
    const styleParams = {
        filters: [],
        filterColors: ['#FF0000'],
        filterMarks: ['circle'],
    ...
    const baseFilter = this.createBaseFilter(styleParams.optionalFilters || []);

    // Generate rules based on analytics results or hylogged status
      ...
      ['sld:StyledLayerDescriptor', { 
        ...
        ['sld:NamedLayer', {},
          ...
          ['sld:UserStyle', {},
            ['sld:Name', {}, styleName],
            ['sld:Title', {}, styleName],
            ['sld:Abstract', {}, 'NVCL Borehole style generated by AuScope Portal'],
            ['sld:IsDefault', {}, '1'],
            baseFilter,
            ...rules
          ]
  ...
  private static createBaseFilter(filters: OptionalFilter[]): string {
    ...
    // Add hylogged filter - always required for NVCL
    filterParts.push(this.createHyloggedFilter());
  ...
  private static createAnalyticsRules(analytics: any, baseFilter: string, ns: any): any[] {
    const rules = [];

    if (analytics.errorBoreholes?.length > 0) {
      rules.push(this.createRule(
        'Error Boreholes',
        this.combineFilters([baseFilter, this.createIdentifierFilter(analytics.errorBoreholes)]),
        '#ff8000',
        'circle',
        ns
      ));
    ...
    if (analytics.failBoreholes?.length > 0) {
      rules.push(this.createRule(
        'Fail Boreholes',
        this.combineFilters([baseFilter, this.createIdentifierFilter(analytics.failBoreholes)]),
        '#cc0000',
        'circle',
        ns
      ));
    ...
    if (analytics.passBoreholes?.length > 0) {
      rules.push(this.createRule(
        'Pass Boreholes',
        this.combineFilters([baseFilter, this.createIdentifierFilter(analytics.passBoreholes)]),
        '#0000ff',
        'circle',
        ns
      ));
